...
export function useKudos() {
  ...
  const { writeContractSponsoredAsync, error, isPending, isSuccess } =
    useWriteContractSponsored();

  const [lastAction, setLastAction] = useState<
    'register' | 'kudos' | 'delete' | 'privacy' | null
  >(null);
  ...
  const checkUserRegistration = useCallback(
    async (userAddress?: Address): Promise<UserData | null> => {
  ...
  const getKudosHistory = useCallback(
    async (
      offset: number = 0,
      limit: number = 10
    ): Promise<KudosTransaction[]> => {
      try {
    ...
    async (limit: number = 10): Promise<LeaderboardEntry[]> => {
      try {
        ...
        if (result && Array.isArray(result) && result.length === 3) {
          const [handles, kudosReceived, addresses] = result as [
            string[],
            bigint[],
            Address[],
          ];
  ...
  const setProfilePrivacy = useCallback(
  ...
  const giveKudos = useCallback(
    async (toHandle: string, tweetUrl: string) => {
      if (!abstractClient) throw new Error('Wallet not connected');
